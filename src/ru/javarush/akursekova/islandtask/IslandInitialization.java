...
//                currentLocation.setAmountAnimalsInLocation(Fox.class, foxAmountInLocation);
                ...
                //TEST-TEST-TEST
//                for (int k = 0; k < 1; k++) {
//                    currentLocation.setAnimals(new Wolf());
//                    PopulationCounter.getInstance().addCarnivore();
//                }
            }
        }
...
//                currentLocation.setAmountAnimalsInLocation(Sheep.class, sheepAmountInLocation);
    ...
    /*
    public void fillCarnivoreAnimals(Island island) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        //ConsoleWriter.getSummaryAnimalsToBeCreated();
        for (int i = 0; i < island.getWidth(); i++) {
            for (int j = 0; j < island.getLength(); j++) {
                for (Map.Entry entry : ANIMALS_POPULATION_CLASSES.entrySet()) {
                    ...
                    if (parentClassName.equals("Carnivore")) {
                        for (int k = 0; k < maxCountOfAnimals; k++) {
                            currentLocation.setAnimals((Animal) currentClass.getConstructor().newInstance());
                            PopulationCounter.getInstance().addCarnivore();
                        }
                        //System.out.println("Создано " + maxCountOfAnimals + " хищников класса " + currentClass.getSimpleName());
                    }
                }
            }
        }
        //System.out.println("Было добавлено " + PopulationCounter.getInstance().getCarnivores() + " хищников на остров.");
    }
     */

    /*public void fillHerbivoreAnimals(Island island) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        for (int i = 0; i < island.getWidth(); i++) {
            for (int j = 0; j < island.getLength(); j++) {
                for (Map.Entry entry : ANIMALS_POPULATION_CLASSES.entrySet()) {
                    Class currentClass = (Class) entry.getKey();
                    ...
                    String parentClassName = currentClass.getSuperclass().getSimpleName();
                    int maxCountOfAnimals = (int) entry.getValue();
                    if (parentClassName.equals("Herbivore")) {
                        for (int k = 0; k < maxCountOfAnimals; k++) {
                            currentLocation.setAnimals((Animal) currentClass.getConstructor().newInstance());
                            PopulationCounter.getInstance().addHerbivores();
                        }
                    }
                }
            }
        }
        //System.out.println("Было добавлено " + PopulationCounter.getInstance().getHerbivores() + " травоядных на остров.");
    }
     */
